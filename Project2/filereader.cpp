...
int filereader::getline( ::MyString& buf, int blank )

   //  This method reads the next line from the input stream, and returns
   //  it to the user as a MyString
   //
...
int filereader::next( MyString buf[], int size, char *sep, char *quo, int eat )
   ...
   //  eat:   Eat consecutive separators flag (default is TRUE)
{
  ...
  //  Strip extra CR on the end of DOS-based lines
        ...
        elem +=
          MyString( line ).token( &buf[ elem ], size - elem, sep, quo, eat );
      }
      while( !done );
   }
...
{
   if ( mode != READ && mode != READ_WRITE && mode !=APPEND ) {
      return 0L;
   }
...
int filereader::open( MyString fname, char m, int binary, int create )
...
void filereader::write( MyString line )
